...
class ServerPromptsTest : AbstractServerFeaturesTest() {
    ...
    fun `Should add a prompt`() = runTest {
        // Add a prompt
        val testPrompt = Prompt(name = "test-prompt-with-custom-handler", description = "Test Prompt")
    ...
    fun `Should remove a prompt`() = runTest {
        // given
        val testPrompt = Prompt(name = "test-prompt-to-remove", description = "Test Prompt")
        server.addPrompt(testPrompt) {
            GetPromptResult(
                description = "Test prompt description",
                messages = listOf(),
        ...
        val mcpException = shouldThrow<McpException> {
            client.getPrompt(GetPromptRequest(name = testPrompt.name))
        }
        mcpException.code shouldBe ErrorCode.Defined.InternalError.code
        mcpException shouldHaveMessage
            "MCP error -32603: \"Prompt not found: 'test-prompt-to-remove'\""
