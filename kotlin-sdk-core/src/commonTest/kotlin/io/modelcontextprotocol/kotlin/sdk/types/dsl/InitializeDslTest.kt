...
class InitializeDslTest {
    ...
    @Test
    fun `InitializeRequest should create request with all fields`() {
    ...
    @Test
    fun `InitializeRequest should support direct capabilities and info`() {
        val capabilities = ClientCapabilities(roots = ClientCapabilities.Roots(listChanged = false))
        val info = Implementation(name = "Direct", version = "0.1")
    ...
    @Test
    fun `InitializeRequest capabilities DSL should support direct JsonObject values`() {
        val samplingObj = buildJsonObject { put("key", "value") }
        val elicitationObj = buildJsonObject { put("key", "value") }
        val experimentalObj = buildJsonObject { put("key", "value") }
    ...
    @Test
    fun `InitializeRequest ClientCapabilitiesBuilder roots should support default arguments`() {
        val request = InitializeRequest {
            protocolVersion = "1.0"
            capabilities {
                roots()
    ...
    @Test
    fun `InitializeRequest should throw if protocolVersion is missing`() {
        shouldThrow<IllegalArgumentException> {
            InitializeRequest {
                capabilities { }
                info("Test", "1.0")
    ...
    @Test
    fun `InitializeRequest should throw if capabilities are missing`() {
        shouldThrow<IllegalArgumentException> {
            InitializeRequest {
                protocolVersion = "1.0"
                info("Test", "1.0")
    ...
    @Test
    fun `InitializeRequest should throw if info is missing`() {
        shouldThrow<IllegalArgumentException> {
            InitializeRequest {
                protocolVersion = "1.0"
                capabilities { }
